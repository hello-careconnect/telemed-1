...
const { faqSection: t } = translations;
const FAQ_ICONS = [Target, Rocket, ClipboardList, Bot, CircleDollarSign, Stethoscope];
...
export const FAQSection = () => {
  ...
  return (
    <section id="about" style={banglaFont} className="py-12 sm:py-16 lg:py-24 bg-soft-img relative overflow-hidden">
          ...
          <div className="lg:w-[42%]">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }}>
              ...
              <h2 className="mt-3 font-heading font-bold text-[32px] sm:text-[40px] text-text-primary leading-[1.12]">
                {tx(t.headingLine1, lang)}
                <br />
                <span className="font-display text-primary">{tx(t.headingLine2, lang)}</span>
              </h2>
              ...
              <div className="mt-8 flex gap-6">
                <div className="flex flex-col items-center">
                  ...
                  <p className="font-body text-[13px] text-text-body">{tx(t.statEarlyLabel, lang)}</p>
                </div>
                <div className="w-px bg-border" />
                <div className="flex flex-col items-center">
                  <p className="font-heading font-bold text-[28px] text-primary">4.9</p>
                  <p className="font-body text-[13px] text-text-body">{tx(t.statRatingLabel, lang)}</p>
                </div>
                <div className="w-px bg-border" />
                <div className="flex flex-col items-center">
                  <p className="font-heading font-bold text-[28px] text-primary">24/7</p>
                  <p className="font-body text-[13px] text-text-body">{tx(t.statSupportLabel, lang)}</p>
                </div>
              </div>
            </motion.div>
          </div>
          ...
          <div className="lg:w-[58%] space-y-2">
            {faqs.map((faq, i) => {
              ...
              return (
                ...
                >
                  <div className={`rounded-2xl border transition-all duration-300 ${isOpen ? 'bg-white border-primary/20 shadow-lg shadow-primary/[0.06]' : 'bg-white/60 border-border hover:bg-white hover:border-primary/10 hover:shadow-md hover:shadow-primary/[0.04]'}`}>
                    <button onClick={() => toggle(i)} className="w-full flex items-center gap-3 px-5 py-4 text-left">
                      <span className={`flex-shrink-0 w-9 h-9 rounded-lg flex items-center justify-center transition-colors duration-300 ${isOpen ? 'bg-primary/10 text-primary' : 'bg-gray-100 text-text-body'}`}>
                      ...
                      <span className={`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center transition-all duration-300 ${isOpen ? 'bg-primary text-white rotate-180' : 'bg-gray-100 text-text-body'}`}>
                      ...
                      {isOpen && (
                        ...
                        >
                          <div className="px-5 pb-4 pl-[3.25rem]">
                            <p className="font-body text-[14px] text-text-body leading-[1.75]">{faq.a}</p>
                          </div>
                        </motion.div>
                      )}
                    </AnimatePresence>
