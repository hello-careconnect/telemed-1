...
import { User, Mail, Loader2, CheckCircle, Stethoscope, BadgeCheck, Lock, Building2, MapPin, ChevronDown, Phone } from 'lucide-react';
import { useLang, tx } from '@/context/LanguageContext';
import { translations } from '@/i18n/translations';

const { waitlistForm: t } = translations;
...
export const WaitlistForm = () => {
  ...
  return (
    <section id="waitlist-form" style={banglaFont} className="relative bg-soft-img py-8 sm:py-12 lg:py-16 overflow-hidden">
      <div className="relative z-10 container max-w-[1140px] mx-auto px-6">
        <div className="text-center mb-8 sm:mb-12">
          ...
          <h2 className="mt-3 font-heading font-bold text-[32px] sm:text-[40px] text-text-primary leading-[1.15]">
            ...
            <span className="text-primary">{tx(t.headingAccent, lang)}</span>
          </h2>
          <p className="mt-3 font-body text-[18px] text-text-muted">
            {tx(t.subBefore, lang)}{' '}
            <span className="font-semibold text-text-body">{tx(t.subAccent, lang)}</span>
            {tx(t.subAfter, lang)}
          </p>
        </div>
        ...
        >
          <form onSubmit={handleSubmit} className={`space-y-5 transition-opacity duration-300 ${status === 'success' ? 'opacity-40 pointer-events-none' : ''}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block font-body font-medium text-[13px] text-text-body mb-1.5">{tx(t.labelName, lang)}</label>
                <div className="relative group">
                  ...
                  <input type="text" placeholder={tx(t.placeholderName, lang)} className="w-full h-11 rounded-lg border border-gray-200 pl-10 pr-3.5 font-body text-[14px] text-text-primary placeholder:text-gray-400 focus:border-primary focus:ring-2 focus:ring-primary/10 outline-none transition-all bg-gray-50/50 focus:bg-white" required />
                </div>
              </div>
              <div>
                <label className="block font-body font-medium text-[13px] text-text-body mb-1.5">{tx(t.labelEmail, lang)}</label>
                <div className="relative group">
                  ...
                  <input type="email" placeholder="your@email.com" className="w-full h-11 rounded-lg border border-gray-200 pl-10 pr-3.5 font-body text-[14px] text-text-primary placeholder:text-gray-400 focus:border-primary focus:ring-2 focus:ring-primary/10 outline-none transition-all bg-gray-50/50 focus:bg-white" required />
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block font-body font-medium text-[13px] text-text-body mb-1.5">{tx(t.labelPhone, lang)}</label>
                <div className="relative group flex">
                  ...
                  <input type="tel" placeholder="01X XXXX XXXX" className="flex-1 h-11 rounded-r-lg border border-gray-200 px-3.5 font-body text-[14px] text-text-primary placeholder:text-gray-400 focus:border-primary focus:ring-2 focus:ring-primary/10 outline-none transition-all bg-gray-50/50 focus:bg-white" />
                </div>
              </div>
              <div>
                <label className="block font-body font-medium text-[13px] text-text-body mb-1.5">{tx(t.labelCity, lang)}</label>
                <div className="relative group">
                  ...
                  <select className="w-full h-11 rounded-lg border border-gray-200 pl-10 pr-9 font-body text-[14px] text-text-primary focus:border-primary focus:ring-2 focus:ring-primary/10 outline-none transition-all bg-gray-50/50 focus:bg-white appearance-none">
                    <option value="">{tx(t.selectCity, lang)}</option>
                    {t.cities.map((c) => (
                      <option key={c.en} value={c.en}>{tx(c, lang)}</option>
                    ))}
                  </select>
                </div>
              </div>
            ...
            {/* Role selector */}
            <div>
              <label className="block font-body font-medium text-[13px] text-text-body mb-2">{tx(t.roleLabel, lang)}</label>
              <div className="grid grid-cols-3 gap-2.5">
                <RoleTile icon={<User className="w-6 h-6" />}         title={tx(t.rolePatient.title, lang)}  desc={tx(t.rolePatient.desc, lang)}  selected={role === 'patient'}  onClick={() => setRole('patient')} />
                <RoleTile icon={<Stethoscope className="w-6 h-6" />}  title={tx(t.roleDoctor.title, lang)}   desc={tx(t.roleDoctor.desc, lang)}   selected={role === 'doctor'}   onClick={() => setRole('doctor')} />
                <RoleTile icon={<Building2 className="w-6 h-6" />}    title={tx(t.roleHospital.title, lang)} desc={tx(t.roleHospital.desc, lang)} selected={role === 'hospital'} onClick={() => setRole('hospital')} />
              </div>
              <AnimatePresence>
                {role === 'doctor' && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25 }} className="overflow-hidden">
                    <div className="mt-3 bg-emerald-50 border border-emerald-200/60 rounded-lg p-3 flex items-center gap-2.5">
                      ...
                      <p className="font-body text-[13px] text-text-body">{tx(t.doctorNote, lang)}</p>
                    </div>
                  </motion.div>
                )}
                {role === 'hospital' && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25 }} className="overflow-hidden">
                    <div className="mt-3 bg-emerald-50 border border-emerald-200/60 rounded-lg p-3 flex items-center gap-2.5">
                      ...
                      <p className="font-body text-[13px] text-text-body">{tx(t.hospitalNote, lang)}</p>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
            ...
            <div className="relative pt-1">
              <button type="submit" disabled={status !== 'idle'} className="w-full bg-primary text-primary-foreground rounded-xl py-3.5 text-[15px] font-semibold font-body hover:bg-primary-dark hover:-translate-y-0.5 hover:shadow-lg hover:shadow-primary/20 transition-all duration-200 flex items-center justify-center gap-2 disabled:opacity-80 active:scale-[0.99]">
                {status === 'idle'    && tx(t.submitIdle, lang)}
                {status === 'loading' && <Loader2 className="w-5 h-5 animate-spin" />}
                {status === 'success' && <span className="flex items-center gap-2"><CheckCircle className="w-5 h-5" />{tx(t.submitSuccess, lang)}</span>}
              </button>
              {particles && (
          ...
          <div className="mt-5 flex flex-col items-center gap-3">
            <p className="font-body text-[12px] text-gray-400 flex items-center gap-1">
              <Lock className="w-3.5 h-3.5" strokeWidth={1.5} /> {tx(t.privacy, lang)}
            </p>
            <div className="w-full max-w-xs">
              <div className="flex justify-between items-center mb-1.5">
                <p className="font-body text-[12px] text-gray-500">
                  <span className="font-semibold text-primary text-[13px]">412</span>{' '}
                  {lang === 'en' ? 'of 500 spots left' : 'টি স্পট বাকি ৫০০টির মধ্যে'}
                </p>
                <p className="font-body text-[11px] text-gray-400">
                  {lang === 'en' ? '18% claimed' : '১৮% ক্লেইম হয়েছে'}
                </p>
              </div>
