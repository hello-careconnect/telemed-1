...
const { doctorsTeam: t } = translations;

const DOCTORS = [
  { name: 'Dr. Aisha Rahman',    specialtyIdx: 0, image: doctorAisha,        objectPosition: 'center center', rating: 4.9, reviews: 127, distance: '1.2 km' },
  { name: 'Dr. Karim Hassan',    specialtyIdx: 1, image: doctorClipboard,    objectPosition: 'center top',    rating: 4.8, reviews: 94,  distance: '3.5 km' },
  { name: 'Dr. Nasreen Sultana', specialtyIdx: 2, image: doctorNasreen,      objectPosition: 'center top',    rating: 4.9, reviews: 203, distance: '2.8 km' },
  { name: 'Dr. Rafiq Ahmed',     specialtyIdx: 3, image: doctorRafiq,        objectPosition: 'center top',    rating: 4.7, reviews: 86,  distance: '4.1 km' },
  { name: 'Dr. Imran Chowdhury', specialtyIdx: 4, image: doctorYoungGlasses, objectPosition: 'center top',    rating: 4.8, reviews: 112, distance: '1.9 km' },
  { name: 'Dr. Karim Uddin',     specialtyIdx: 5, image: doctorKarimUddin,   objectPosition: 'center top',    rating: 4.6, reviews: 71,  distance: '5.3 km' },
];
...
if (typeof window !== 'undefined') {
  DOCTORS.forEach((doc) => { const img = new Image(); img.src = doc.image; });
}
    ...
    <section style={banglaFont} className="py-8 sm:py-12 lg:py-16 bg-soft-img">
      <div className="container max-w-[1140px] mx-auto px-6">
        ...
        <div className="text-center mb-10 sm:mb-14">
          ...
          <h2 className="mt-1 font-heading font-bold text-[28px] sm:text-[40px] text-text-primary leading-[1.12]">
            {tx(t.headingBefore, lang)}{' '}
            <span className="font-display text-primary">{tx(t.headingAccent, lang)}</span>{' '}
            {tx(t.headingAfter, lang)}
          </h2>
            ...
            {visibleDoctors.map((doc, i) => (
              ...
              >
                ...
                <div className="flex items-center gap-1 mb-3">
                  ...
                  <span className="font-body text-[11px] text-primary font-medium">{tx(t.verified, lang)}</span>
                </div>
                ...
                <p className="font-body text-[14px] mb-3 text-primary">
                  {tx(t.specialties[doc.specialtyIdx], lang)}
                </p>
        ...
        <div className="flex flex-col items-center gap-6 mt-10">
          <div className="flex gap-3">
            <button onClick={prev} className="w-10 h-10 rounded-full border border-border hover:border-primary hover:text-primary flex items-center justify-center transition-colors">
              <ChevronLeft className="w-4 h-4" />
            </button>
            <button onClick={next} className="w-10 h-10 rounded-full border border-border hover:border-primary hover:text-primary flex items-center justify-center transition-colors">
              <ChevronRight className="w-4 h-4" />
            </button>
          </div>
          <button className="font-body text-[14px] text-primary hover:underline underline-offset-4 transition-colors">
            {tx(t.checkOthers, lang)}
          </button>
        </div>
      </div>
    </section>
